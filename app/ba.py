...
def create_venue_cards(venues):
    cards_html = '<div style="display: flex; overflow-x: auto; background-color: #eef2f7;">'
    for name, address in venues:
        card_html = f"""
            <div style="min-width: 200px; margin: 10px; padding: 20px; display: flex; flex-direction: column; justify-content: center; align-items: center; background-color: #d1e0eb; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,.2);">
                <h4 style="margin: 0; font-family: 'Arial', sans-serif; font-weight: bold;">{name}</h4>
                <p style="margin: 7px 0; font-size: 0.9em;">{address}</p>
            </div>
        """
        cards_html += card_html
    cards_html += '</div>'
...
def create_provider_cards(providers):
    cards_html = '<div style="display: flex; overflow-x: auto; background-color: #f7eef2;">'
    for name, theme in providers:
        card_html = f"""
            <div style="min-width: 200px; margin: 10px; padding: 20px; display: flex; flex-direction: column; justify-content: center; align-items: center; background-color: #ebd1e0; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,.2);">
                <h4 style="margin: 0; font-family: 'Arial', sans-serif; font-weight: bold;">{name}</h4>
                <p style="margin: 7px 0; font-size: 0.9em;">Theme: {theme}</p>
            </div>
        """
        cards_html += card_html
    cards_html += '</div>'
